...
namespace lut {
bool CubeLUT::load(const std::filesystem::path& path) {
    ...
    while (std::getline(file, line)) {
        ...
        if (token == "TITLE") {
            ...
            if (!(iss >> domain_min.r >> domain_min.g >> domain_min.b)) return false;
        } else if (token == "DOMAIN_MAX") {
            if (!(iss >> domain_max.r >> domain_max.g >> domain_max.b)) return false;
        } else if (token == "LUT_1D_SIZE") {
            dimension = 1;
            iss >> size;
            if (size < 2u || size > 65536u) return false;

            capacity = size;
        } else if (token == "LUT_3D_SIZE") {
            dimension = 3;
            iss >> size;
            if (size < 2u || size > 256u) return false;
    ...
    while (std::getline(file, line)) {
        ...
        std::istringstream iss(line);
        float r, g, b;
        if (iss >> r >> g >> b) tmp.push_back({r, g, b});
    }

    if (tmp.size() != capacity) return false;

    pixel::RGBF32 range = {domain_max.r - domain_min.r, domain_max.g - domain_min.g, domain_max.b - domain_min.b};
    if (range.r < eps || range.g < eps || range.b < eps) return false;
...
bool HaldCLUT::load(const std::filesystem::path& path) {
    using Microsoft::WRL::ComPtr;
    ...
    const auto target = is_hdr ? GUID_WICPixelFormat96bppRGBFloat : GUID_WICPixelFormat48bppRGB;
    ...
    if (w != h || w < 8u || w != level * level * level) return false;
...
bool HaldCLUT::export_cube(const std::filesystem::path& path, const std::u8string& title) const {
    size_t size = level * level * level;
    size *= size;
    if (level < 2u || data.size() != size) return false;
    ...
    if (!title.empty()) file << "TITLE \"" << string::as_string(title) << "\"\n";
...
bool HaldCLUT::export_png(const std::filesystem::path& path, const std::u8string& title) const {
    ...
    if (level < 2u || data.size() != capacity) return false;
