...
namespace lut::pixel {
void to_rgb16(RGB16* dst, const RGBF32* src, size_t w, size_t h) {
    ...
    std::for_each(std::execution::par, idx.begin(), idx.end(), [&](size_t y) {
        ...
        while (x + 8uz <= len) {
            ...
            const __m128i u16 = _mm_packus_epi32(_mm256_castsi256_si128(i32), _mm256_extractf128_si256(i32, 1));
...
void to_rgbaf32(RGBAF32* dst, const RGBAF16* src, size_t w, size_t h) {
    const auto idx = std::views::iota(0uz, h);
...
void to_rgbf32(RGBF32* dst, const RGB16* src, size_t w, size_t h) {
